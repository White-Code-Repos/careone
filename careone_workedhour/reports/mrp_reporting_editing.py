...
class MrpCostEditing(models.AbstractModel):
    ...
    def get_lines(self, productions):
        ...
        for product in productions.mapped('product_id'):
            ...
            if Workorders:
                query_str = """SELECT w.operation_id, op.name,u.id, partner.name, sum(t.duration), wc.costs_hour
                                ...
                                GROUP BY w.operation_id, op.name,u.id, partner.name, t.user_id, wc.costs_hour
                                ORDER BY op.name, partner.name
                            """
                self.env.cr.execute(query_str, (tuple(Workorders.ids), ))
                for op_id, op_name, user_id , user, duration, cost_hour in self.env.cr.fetchall():
                    user_for_cost = self.env['res.users'].search([('id', '=', user_id)])
                    emp_related = self.env['hr.employee'].search([('id', '=', user_for_cost.employee_id.id)])
                    cost_per_hour = emp_related.timesheet_cost

                    operations.append([user, op_id, op_name, duration / 60.0, cost_per_hour])
